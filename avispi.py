...
canvas = Canvas(mainframe, width = 1200, height = 637, background = "grey")
...
def siguiente_img(event):
	...
	match canal_activo:
		case 0:
			imgMapa = Image.open('defecto/fondo.png') # Fondo por defecto
			imgMapa = ImageOps.fit(imgMapa, (1200, 637))
			imgMapa = ImageTk.PhotoImage(imgMapa)
			canvas.delete('all')
			canvas.create_image(0, 0, image = imgMapa, anchor = 'nw' )
...
def anterior_img(event):
	...
	match canal_activo:
		case 0:
			imgMapa = Image.open('defecto/fondo.png')
			imgMapa = ImageOps.fit(imgMapa, (1200, 637))
			imgMapa = ImageTk.PhotoImage(imgMapa)
			canvas.delete('all')
			canvas.create_image(0, 0, image = imgMapa, anchor = 'nw' )
...
def predecir():
	...
	# Solo predice las listas de nubes que tengan dos o más elementos.
...
root.bind('<BackSpace>', borra_ultimo)
